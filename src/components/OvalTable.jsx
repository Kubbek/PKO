          ...
          if (p) {
            return (
              ...
              >
                ...
                <div style={{ fontSize: 16, fontWeight: 700, color: isWinner ? 'var(--green,#059669)' : isLoser ? 'var(--red,#dc2626)' : 'var(--text,#1c1917)', lineHeight: 1.25, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', paddingTop: 2 }}>
                  {p.name}{p.rebuys > 1 && <span style={{ fontSize: 8, color: 'var(--accent,#b45309)', marginLeft: 2, fontWeight: 700 }}>R{p.rebuys}</span>}
                </div>
                <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 16, color: isWinner ? 'var(--green,#059669)' : isLoser ? 'var(--red,#dc2626)' : 'var(--accent,#b45309)', letterSpacing: 0.5, marginTop: 2, lineHeight: 1 }}>
                  {r2(p.bounty)} zł
                </div>
              </div>
            )
